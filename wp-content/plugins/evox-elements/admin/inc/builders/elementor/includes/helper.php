...
defined('Evox_Elements_ELEMENTOR') or exit();

class Evox_Elements_Elementor_Helper{
...
//        }
//
//        $base_path      = Evox_Elements_ELEMENTOR_WIDGETS_PATH.'/'.$widget_base.'/tmpl/'.$template_name;
...
//
//        var_dump(__METHOD__);
//        var_dump(Evox_Elements_ELEMENTOR_WIDGETS_PATH);
//        var_dump($base_path);
...
//        require $template_path;
    ...
    public static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
        ...
        if ( ! $default_path ) {
            $default_path = trailingslashit(Evox_Elements_BUILDERS_WIDGETS_PATH ). $template_path; // Path to the template folder
        }
        ...
        $child_template_path = trailingslashit(Evox_Elements_BUILDERS_WIDGETS_PATH ). $base . "/" . $template_name;
        if ( file_exists( $child_template_path ) ) {
            $template = $child_template_path;
        }

        return apply_filters( 'evox-elements-builder/locate-template', $template, $template_name, $template_path, $default_path );
    }
	...
	public static function get_attachment_image_html( $settings, $image_size_key = 'image', $image_key = null, $image_class = '' ) {
		 ...
		 *                               as the image key.
		 */
		return apply_filters( 'evox-elements/get_attachment_image_html', $html, $settings, $image_size_key, $image_key );
	}
}