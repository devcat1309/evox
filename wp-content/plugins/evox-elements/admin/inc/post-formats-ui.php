...
// but this gives us the info we need to get our stuff in.
    function vp_pfui_add_meta_boxes($post_type) {
        if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
            // assets
            wp_enqueue_script('vp-post-formats-ui', PLUGIN_PATH.'assets/js/post-formats.js', array('jquery'), VP_PFUI_VERSION);
            wp_enqueue_style('vp-post-formats-ui', PLUGIN_PATH.'assets/css/post-formats.css', array(), VP_PFUI_VERSION, 'screen');
            wp_localize_script(
                'vp-post-formats-ui',
                'vp_pfui_post_format',
                array(
