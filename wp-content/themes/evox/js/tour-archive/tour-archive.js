 ...
 * @variables class
 */
window.addEventListener("load", evox_tour_page_init, false);
function evox_ResizeImage(obj){
    var widthStage;
    var heightStage ;
    var widthImage;
    var heightImage;
...
/*  Creat Cookie  */
function evox_createCookie(name,value,days) {
    if (days) {
        var date = new Date();
...
/*  Delete Cookie  */
function evox_eraseCookie(name) {
    evox_createCookie(name,"",-1);
}

function evox_tour_page_init(){
    ...
    var tour_layouts = evox_readCookie('tour_layout');
    ...
    var $grid = jQuery('.tour-masonry').imagesLoaded( function() {
        ...
        evox_ResizeImage(jQuery('.tz-thumb'));
    });
    jQuery('.tz-tour-item').css({
        opacity:1
    });
...
jQuery(window).load(function() {
    ...
    tz_resizeTimer = setTimeout("evox_tour_page_init()", 150);
});
...
jQuery(window).resize(function() {
    ...
    if (tz_resizeTimer) clearTimeout(tz_resizeTimer);
    tz_resizeTimer = setTimeout("evox_tour_page_init()", 100);
});
...
jQuery(document).ready(function(){
    ...
    jQuery('.tour-layout-list-btn').on('click', function(){
        ...
        if(active_class==false){
            jQuery(this).parent().find('.type-btn').removeClass('active');
            jQuery(this).addClass('active');
            evox_createCookie('tour_layout','list',1);
            jQuery('.tour-masonry').addClass('tour-layout-list');
