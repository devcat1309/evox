    ...
    window.inputNumber = function(el) {
        ...
        function init(el) {
            ...
            els.dec.on('click', function(){
                decrement();
                evox_check_tour_available_people();
                evox_update_tour_price( jQuery(this).parent().find('.input-number') );
            });
...
function evox_check_allow_select_people(){
    ...
    if ( booking_form.find('input.date-pick').length && booking_form.find('select[name="departure_time"]').length ) {
        ...
        if(tour_date && tour_time){
            ...
            evox_tour_check_availability_ajax();
...
function evox_tour_check_availability_ajax(){
    var booking_form = jQuery('.tz-tour-booking');
    ...
    jQuery.ajax({
        ...
        type: 'POST',
        data: ({
            action: 'evox_tour_check_availability_ajax',
            tour_id: tour_id,
            tour_date: tour_date,
            tour_time: tour_time
        }),
...
function evox_check_tour_available_people(){
    var booking_form = jQuery('.tz-tour-booking');
...
/*  Get Price Value   */
function evox_update_tour_price( obj ) {
    ...
    if( adults_number >= 0 && kids_number >= 0 ){
        ...
        /*  format price   */
        total_adults_price = evox_number_format(total_adults_price,decimal_prec,decimal_sep,thousands_sep);
        total_child_price = evox_number_format(total_child_price,decimal_prec,decimal_sep,thousands_sep);
        total_all_price = evox_number_format(total_all_price,decimal_prec,decimal_sep,thousands_sep);
...
function evox_update_combo_tour_price( obj ) {
    var booking_form    = obj.closest('.tz-tour-booking');
    var tour_data       = booking_form.find(".tz-tour-data");
    ...
    if( price_new != 'custom' ){
        ...
        if( price_new >= 0 ){
            /*  format price   */
            price_new = evox_number_format(price_new,decimal_prec,decimal_sep,thousands_sep);
            /*  replace text   */
            if(thousands_sep===' '){
        ...
        if(thousands_sep===' '){
            var text_all_price_0 = booking_form.find(".form-group.has_combo .total_price_adults").text().replace(/[\d\ \.]+/g, evox_number_format('0',decimal_prec,decimal_sep,thousands_sep));
        }else{
            var text_all_price_0 = booking_form.find(".form-group.has_combo .total_price_adults").text().replace(/[\d\.\,]+/g, evox_number_format('0',decimal_prec,decimal_sep,thousands_sep));
        }
        ...
        /*  format price   */
        price_old = evox_number_format(price_old,decimal_prec,decimal_sep,thousands_sep);
        /*  replace text   */
        if(thousands_sep===' '){
...
/*  Number Format   */
function evox_number_format (number, decimals, dec_point, thousands_sep) {
