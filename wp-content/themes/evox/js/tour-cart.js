...
jQuery(document).ready(function(){
    ...
    jQuery('#tour-cart select').change(function(){
        ...
        evox_check_tour_available_people_cart();
    });

    jQuery('.update-cart-btn').on("click", function(e){
        e.preventDefault();
        jQuery('input[name="action"]').val('evox_tour_update_cart');
        jQuery.ajax({
            url: evox_ajax.url,
            type: "POST",
    ...
    jQuery('.delete-cart-btn').on("click", function(e){
        e.preventDefault();
        jQuery('input[name="action"]').val('evox_tour_delete_cart');
        jQuery.ajax({
            url: evox_ajax.url,
            type: "POST",
            data: jQuery('#tour-cart').serialize(),
    ...
    jQuery('.book-now-btn').on("click", function(e){
        ...
        if ( is_woocommerce_enabled == 1 ) {
            jQuery('input[name="action"]').val('evox_add_tour_to_woo_cart');
            jQuery.ajax({
                url: evox_ajax.url,
                type: "POST",
                data: jQuery('#tour-cart').serialize(),
...
function evox_check_tour_available_people_cart(){
